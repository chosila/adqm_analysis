...
for numref, marker, color in zip(['1_REF', '4_REF', '8_REF'], ['-rD', '-bo', '-g^'], ['purple', 'yellow', 'orange']):
    ...
    cutsIndex = None
    #cuts_df = pd.DataFrame(cuts, columns=df.columns, index=)
    print(cuts)
    print(type(cuts))
    ...
    ax0.annotate(algo + " HF ROC", xy=(0.05, 0.95), xycoords='axes fraction', xytext=(10, -10), textcoords='offset points', ha='left', va='top', fontsize=12, weight='bold')
    ...
    for N, m, c in zip([1,3,5], ['-rD', '-bo', '-g^'], ['purple', 'yellow', 'orange']):
        ...
        ax1.set_xlabel(f'Fraction of good runs with at least N={N} histogram flags')
        ax1.set_ylabel(f'Fraction of bad runs with at least N={N} histogram flags')
        algorithm_name = algo
        ...
        ax1.plot(perc_g, perc_b, m, mfc=c, mec='k', markersize=8, linewidth=1, label=numref)
        ...
        fig1.savefig(plotdir + "RF_ROC_comparison_" + algorithm_name + "_" + numref + ".pdf",bbox_inches='tight')
        print("SAVED: " + plotdir + "RF_ROC_comparison_" + algorithm_name + "_"+ numref  + ".pdf")
